import { Space, Typography, Button, Popconfirm, Popover, Select } from 'antd';
import {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '12px 14px' }}>
        ...
        {card.contentSnippet && (
          <div style={{
            fontSize: 11.5, lineHeight: 1.45, color: 'var(--color-ink-tertiary)',
        ...
        {!isOverlay && hasChecklist && (
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
              ...
              <span style={{
                fontSize: 10.5, color: 'var(--color-ink-tertiary)',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            ...
            {card.dueDate && (
              <span style={{
                fontSize: 10.5, color: isOverdue ? 'var(--color-coral)' : 'var(--color-ink-tertiary)',
          ...
          {!isOverlay && (
            <Space size={0} onClick={(e) => e.stopPropagation()} style={{ flexShrink: 0, opacity: 0, transition: 'opacity 0.15s ease' }} className="card-actions">
              {targetLists.length > 0 && (
                <Popover trigger="click" content={
                  <div style={{ width: 150 }}>
                    <Select size="small" style={{ width: '100%' }} placeholder="移动到"
                      options={targetLists.map((l) => ({ label: l.name, value: l.id }))}
                      onChange={(val) => onMove(val)} />
                  </div>
                }>
                  <Button type="text" size="small" icon={<SwapOutlined />} style={{ fontSize: 11, color: 'var(--color-ink-disabled)' }} />
                </Popover>
              )}
